const d = new Date();
let year = d.getFullYear();
const experience = year - 2015

export const HEADER = {
    name: "Jill Hoenig",
    position: "Senior Engineer",
    about: `Senior Web Engineer with ${experience} years experience leading web projects from inception to completion, focusing on scalability and innovative solutions to complex challenges. Two years experience leading dev teams. Committed to fostering a culture of continuous learning and instilling pride in our work.`
}
export const RESUME = [
    {
        title: 'Skills',
        content: [
            {
                suptext: "LANGUAGES",
                list: ['JavaScript', 'PHP', 'Ruby', 'Liquid', 'CSS / SASS / SCSS', 'HTML']
            },
            {
                suptext: "FRAMEWORKS",
                list: ['React', 'Rails', 'Jekyll']
            },
            {
                suptext: "TOOLS",
                list: ['Git', 'Gulp', 'Webpack', 'Node', 'AWS']
            }
        ]
    },
    {
        title: 'Experience',
        content: [
            {
                break: true,
                isList: true,
                sectionHead: "Ideometry",
                title: "Senior Engineer",
                subtitle: "DECEMBER 2021 - MARCH 2025, REMOTE",
                copy: 'Led web project development, ensuring adherence to coding standards and efficient practices. Implemented continuous integration processes to streamline team collaboration. Fostered a productive and enjoyable work environment, emphasizing high-quality deliverables. Developed and implemented a professional development program for junior developers, focusing on key skills such as Git, command-line usage, and JavaScript.'
            },
            {
                isList: true,
                title: "Founding Engineer",
                subtitle: "MAY 2017 - DECEMBER 2021, BOSTON MA - REMOTE",
                copy: 'Executed the full lifecycle of website projects, including server setup and QA testing, to meet client expectations. Collaborated with design team to deliver innovative websites that garnered industry recognition, including the CUNA Diamond Award winning Quorum Learning Hub. Focused on creating efficient, user-friendly websites with scalability to adapt to evolving client needs and industry trends.',
            },
            {
                isList: true,
                sectionHead: "Isovera",
                title: "Frontend Engineer",
                subtitle: "NOVEMBER 2015 - MAY 2017, WALTHAM MA",
                copy: "Improved user experience of company's internal web platform through strategic improvements and user-centered design principles, such as adding icons and ensuring ADA compliance. Provided key development support and technical expertise to strengthen the dev team."
            }
        ]
    },
    {
        title: "Contact",
        content: [
            {
                contact: ['(201) 562-5969', 'hello@jill.digital']
            },
            {
                suptext: "PORTFOLIO",
                contact: ['jill.digital'],
                isLink: true
            },
            {
                suptext: "REPOS",
                contact: ['github.com/jillii', 'gitlab.com/jillii'],
                isLink: true
            },
            {
                suptext: "BLOG",
                contact: ['medium.com/@jillii'],
                isLink: true
            }
        ]
    },
    {
        title: 'Background',
        content: [
            {
                suptext: "SIDE PROJECTS",
                sectionHead: "jillii.github.io/guess-the-phrase",
                title: '"Wheel of Fortune" app',
                copy: "Interactive React application where users guess a random phrase, one letter at a time.",
                link: "https://jillii.github.io/guess-the-phrase/"
            },
            {
                sectionHead: "jillii.github.io/resume",
                title: "React app resume",
                copy: 'Yes, this resume is a React app (although you might be viewing the PDF version). Reactified resume for organization and easy updates. Automatically calculates years of experience by comparing current year with year of first experience.',
                link: "https://jillii.github.io/resume/"
            },
            {
                sectionHead: "expletive.plumbing",
                title: "Portfolio site for IDM project !@#$%",
                copy: 'Original design with multiple skins. Fully functional media player built with vanilla JavaScript, featuring shuffle play, draggable playback slider, and filter play functionality. Built with flat database structure to ensure lightning-fast page load.',
                link: "https://expletive.plumbing"
            },
            {
                sectionHead: "cakefactory.party",
                title: "Official site for rave-organizing collective",
                copy: "Comprehensive platform featuring information on releases and upcoming shows. I added video streaming and live chat functionality to host online performances during COVID.",
                link: "https://cakefactory.party"
            },
            {
                sectionHead: "designstatements-llc.com",
                title: "Home for Design Statements LLC",
                copy: "Bespoke WordPress site for an interior designer client featuring custom theme inspired by paint chips.",
                link: "http://designstatements-llc.com"
            },
            {
                suptext: "EDUCATION",
                sectionHead: "General Assembly",
                title: "Backend Development with Ruby on Rails",
                subtitle: "JUNE 2015 - AUGUST 2015,  NEW YORK NY"
            },
            {
                sectionHead: "Brandeis University",
                title: "B.A. in Computer Science",
                subtitle: "SEPTEMBER 2011 - MAY 2015, WALTHAM MA"
            }
        ]
    }
]
